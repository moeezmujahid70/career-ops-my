...
Default modes are in `modes/` (English). German modes are available for DACH market:
...
**When NOT to:** If the user applies to English-language roles, use the default English modes — *unless* the user has explicitly requested German mode in this conversation, or `language.modes_dir` is set in `config/profile.yml` (the explicit user preference always wins over JD-language detection).
